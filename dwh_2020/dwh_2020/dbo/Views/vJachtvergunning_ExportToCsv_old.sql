...
CREATE view [dbo].[vJachtvergunning_ExportToCsv_old]
...
SELECT top 10
  ...
  dbo.CleanString(INDR.TELEFOON)			AS [Tel nr Aanvrgr],
  dbo.CleanString(INDR.Email)				AS [E-mail  Aanvrgr],
  J.JachtVergunningNummer					AS [JachtVergun. Nr],
  J.JachtVerlofNummerAanvrager				AS [JachtVerlof Nr Aanvrgr],
  ...
  dbo.CleanString(GNDGD.TELEFOON)						AS [Tel Nr Genod.],
  ...
  CASE 
    ...
    ELSE 'Nee'
  END										AS [Kopie buitenlands jachtverlof],
  CASE 
	WHEN ISNULL(IAJaExGenod.[FileName],'') = '' THEN 'Nee' 
	ELSE 'Ja' 
  END										AS [Kopie buitenlands jachtverlof aanwzg],
  ...
  CASE j.[BijlageBetalingFlag]						
    ...
    WHEN 0 THEN 'Nee'
    ELSE 'Nee'
  END										AS [Bewijs Vlaamse belast.],
  CASE 
    WHEN LEN(IABEWBET.FileName) > 0 THEN 'Ja'
    ELSE 'Nee'
  END										AS [Bewijs Vlaamse belast. aanwzg],
